(function(cropper, undefined) {
	...
	cropper.viewImage = function(channel){
		if(url === null){
			return false;
		}

		var e = document.getElementById("resolutions");
		var resolution = e.options[e.selectedIndex].value;

		var url = cropper.getCroppedImageSrc();
		if(url == false){
			return false;
		}else{
			cropper.previewImage(url, resolution, channel);
		}
	}

	cropper.previewImage = function(url, resolution, channel){
			...
			else
			{
				...
				if(channel == "R"){
					...
					for (i = 0; i < imgData.data.length; i += 4) {
						imgData.data[i] = 255 - (255 - imgData.data[i]);
						imgData.data[i+1] = 0;
						imgData.data[i+2] = 0;
					...
					for (i = 0; i < imgData.data.length; i += 4) {
						imgData.data[i] = 0;
						imgData.data[i+1] = 255 - (255 - imgData.data[i+1]);
						imgData.data[i+2] = 0;
						imgData.data[i+3] = 255;
					}
				}else if(channel == "B"){
					...
					for (i = 0; i < imgData.data.length; i += 4) {
						imgData.data[i] = 0;
						imgData.data[i+1] = 0;
						imgData.data[i+2] = 255 - (255 - imgData.data[i+2]);
						imgData.data[i+3] = 255;
					}
				}else if(channel == "GRAY"){
					var i;
					for (i = 0; i < imgData.data.length; i += 4) {
						var pixel = (imgData.data[i] + imgData.data[i+1] + imgData.data[i+2])/3
